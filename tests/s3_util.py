...
class S3Util:
    ...
    def _upload_files(self):
        for bucket_name in self._get_bucket_names_to_create():
            ...
            for local_file_path in self._get_file_paths_in_directory_path(
                bucket_name_local_path
            ):
                # print (f"  S3Util._upload_files() upldading {local_file_path} to {bucket_name}")
                s3_file_path_name = str(
                    local_file_path.relative_to(bucket_name_local_path)
                )
                modified_date = datetime.datetime.fromtimestamp(
                    local_file_path.stat().st_mtime, tz=datetime.timezone.utc
                )
                self._upload_file(
                    bucket_name, local_file_path, s3_file_path_name, modified_date
                )
    ...
    def _upload_file(
        ...
        # Add metadata parameter if modified_date is provided
        if modified_date:
            self._s3_client.upload_file(
                Filename=str(local_file_path),
                Bucket=bucket_name,
                Key=s3_file_path_name,
                ExtraArgs={"Metadata": {"last-modified": modified_date.isoformat()}},
            )
        else:
            self._s3_client.upload_file(
                Filename=str(local_file_path), Bucket=bucket_name, Key=s3_file_path_name
            )
